...
Toda mudanca no banco (criar tabela, adicionar coluna...) vira um **arquivo de migration**:
um `.sql` com data no nome, dentro de `supabase/migrations/`.
Como e arquivo, vai pelo Git — e o push faz o deploy.

A gente **nao escreve esse SQL na mao**: o schema fica em TypeScript (Drizzle) e a migration e gerada.

### 1. Editar o schema do modulo

O schema de cada modulo fica em `apps/api/src/db/schema/<modulo>.ts` (um arquivo por modulo —
`clients.ts`, `metrics.ts`, `sla.ts`...). Crie ou altere a tabela ali, em TypeScript:
...
Arquivo novo? Adicione a linha `export * from "./<modulo>";` em `apps/api/src/db/schema/index.ts`.

### 2. Gerar a migration

```powershell
pnpm --filter @inovaapss/api db:generate
```
...
Abra o `.sql` gerado e leia. Confira se e so o que voce queria (uma coluna a mais, uma tabela
renomeada por engano ou um `drop` aparecem aqui). `pnpm --filter @inovaapss/api db:check` confere
se as migrations batem com o schema.
...
**Migration ja commitada nao se edita**: precisa mudar, gera outra.
...
<details>
<summary>SQL na mao (RLS, policies, funcoes, dados iniciais)</summary>

Para coisas que o Drizzle nao gera — policies de RLS, funcoes, triggers, um `insert` de seed —
crie a migration vazia e escreva o SQL nela:

```powershell
npx supabase migration new enable_rls_on_plans
```

Gera `supabase/migrations/20260919120000_enable_rls_on_plans.sql`. Escreva o SQL, revise e `Ctrl+Shift+B`.
</details>
...
Depois: `pnpm deploy` (ou a task **Deploy manual no Supabase**). Normalmente **nao precisa**.
