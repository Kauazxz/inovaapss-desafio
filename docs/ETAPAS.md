...
| 1     | Auth + multi-tenant (Supabase Auth, organizations, organization_users, RBAC, middleware de tenant, login, layout privado)             | P0                         | ✅ concluída — `8defa10`                                                                                        | trilha auth                      | 0                                                                       | `apps/api/src/modules/auth/`, `apps/api/src/modules/organizations/`, `apps/api/src/middleware/` (auth, tenant, rbac, http-errors), `apps/api/src/db/schema/organizations.ts`, `apps/api/src/infrastructure/supabase.ts`, `apps/api/src/db/seed/demo-auth.ts`, `supabase/migrations/*organizations*`, `apps/web/src/features/auth/`, `apps/web/src/routes/`, `apps/web/src/lib/supabase.ts`, `apps/web/src/lib/api.ts`, `apps/web/src/components/layout/PrivateLayout.tsx`, `docs/AUTH.md` | 4 e 5 (lógica pura em `packages/engine`), 8 (com mock), 14 (contas) |
| 2     | Clientes, planos e contratos (CRUD, valor mensal, segmento, porte, status)                                                            | P0                         | ✅ concluída — `75d6826` (integrada em `f95c989`)                                                               | trilha clientes                  | 1 (middleware de tenant)                                                | `apps/api/src/modules/portfolio-clients/`, `apps/api/src/modules/contracts/` (planos e contratos), `apps/api/src/db/schema/clients.ts`, `apps/api/src/db/schema/contracts.ts`, `supabase/migrations/*clients*`, `apps/web/src/features/clients/` (lista e formulário), `apps/web/src/features/contracts/`                                                                                                                                                                                 | 3, 4, 5, 8                                                          |
| 3     | Motor genérico de métricas (definições, tipos, direção, peso, normalização, thresholds, gatilhos, versionamento)                      | P0                         | ✅ concluída — `ad9b1ee` (integrada em `f95c989`)                                                               | trilha metrics                   | 1                                                                       | `apps/api/src/modules/metrics/`, `apps/api/src/db/schema/metrics.ts` (definitions, models, versions, items, values), `supabase/migrations/*metrics*`, `packages/shared/src/domain.ts` (enums de métrica: tipos, direções, fontes — já existe), `packages/validation/src/metrics.ts`, `apps/web/src/features/metrics/` (lista e detalhe simples)                                                                                                                                           | 2, 4 e 5 (puros), 8                                                 |
| 4     | Scoring Engine (current, trend, persistence, metric health, overall, confidence, risk, classes)                                       | P0                         | 🔄 parte pura concluída em `packages/engine` (`84b48f2`) — acoplamento ao banco pendente                        | trilha engine (parte pura)       | lógica pura: 0 · acoplamento ao banco: 3 (e 2 para o impacto comercial) | `packages/engine/src/scoring/` e `packages/engine/src/shared/` (puro, sem banco; inclui `scoring/forecast.ts`), `apps/api/src/modules/scoring/`, `apps/api/src/db/schema/scores.ts` (snapshots), `supabase/migrations/*scores*`, `docs/SCORING.md`, `docs/METRICS_ENGINE.md`                                                                                                                                                                                                              | 1, 2, 3, 5, 8                                                       |
| 5     | SLA Engine (severidades, tipos de chamado, políticas, SLA contratual, meta operacional, consumo, ticket SLA health)                   | P1                         | 🔄 parte pura concluída em `packages/engine` (`84b48f2`) — acoplamento ao banco pendente                        | trilha engine (parte pura)       | lógica pura: 0 · acoplamento ao banco: 2 (contratos/planos)             | `packages/engine/src/sla/` (puro), `apps/api/src/modules/sla/`, `apps/api/src/db/schema/sla.ts` (severities, ticket_types, sla_policies, tickets), `supabase/migrations/*sla*`, `apps/web/src/features/sla/` (configurador §42), `docs/SLA_ENGINE.md`                                                                                                                                                                                                                                     | 1, 2, 3, 4, 8                                                       |
...
| 7     | Importador (XLSX, CSV, JSON, mapeamento, validação, preview, import job, relatório de erros)                                          | P0                         | 🔄 núcleo puro pronto em `packages/importer` (`43b5562`) — módulo `imports` da API e tela `/import` na Rodada 3 | trilha importer                  | 2 + 3                                                                   | `apps/api/src/modules/imports/`, `apps/api/src/db/schema/imports.ts`, `supabase/migrations/*imports*`, `apps/web/src/features/import/`, `docs/IMPORTS.md`                                                                                                                                                                                                                                                                                                                                 | 6, 8, 9, 11                                                         |
| 8     | Dashboard (Em risco, Geral, ranking, health/risk/confidence, evidências, drill-down, forecast priorizado A2)                          | P0 (Em risco) / P1 (Geral) | 🔄 UI com mock concluída (`22020d9`) — ligar à API quando 4/6 fecharem                                          | trilha dashboard                 | começa com mock após 0 · liga em 4 + 6                                  | `apps/web/src/features/dashboard/`, `apps/web/src/components/charts/`, `apps/web/src/lib/chart-theme.ts`, `apps/web/src/lib/mock/dashboard.ts`, `packages/shared/src/dashboard/` (forecast, risk, general, filters — tipos de DATAVIZ §5.4), `apps/api/src/modules/dashboard/`. A projeção pura ficou em `packages/engine/src/scoring/forecast.ts` (Etapa 4) e consome os tipos do shared                                                                                                 | 1, 2, 3, 4, 5, 6, 7, 9 (telas diferentes)                           |
| 9     | Cliente individual (overview, abas, histórico, timeline, evidências, recomendações)                                                   | P0                         | 🔄 UI com mock concluída (`07cf297`) — ligar à API (módulo `client-health`) na Rodada 3                         | trilha client-detail             | 8 (ou junto, telas diferentes) · dados reais: 4 + 6                     | `apps/web/src/features/client-detail/`, `apps/api/src/modules/client-health/` (rotas `/clients/:id/history`, `/scores`, `/evidence`, `/recommendations`)                                                                                                                                                                                                                                                                                                                                  | 8, 7, 10                                                            |
...
| 11    | Arquivo da organização + descoberta de métricas (uploads e planilhas importadas, extração, análise automática, sugestões e aprovação) | P0                         | ✅ funcional — arquivo unificado, análise local sem chave e Claude quando configurado                           | trilha documentos                | 1 + 7 para receber planilhas importadas                                 | `apps/api/src/modules/documents/`, `apps/api/src/infrastructure/storage/`, `apps/api/src/infrastructure/extraction/`, `apps/api/src/db/schema/documents.ts`, `supabase/migrations/*documents*`, `apps/web/src/features/documents/`                                                                                                                                                                                                                                                        | 7 (roda junto), 8, 9, 10                                            |
...
| 19/09/2026 | 1              | Etapa 1 concluída: Supabase Auth na API (requireAuth, tenant, RBAC), módulos auth e organizations, migrations de organizations com RLS, login/recuperação de senha/onboarding no web (`8defa10`). Migrations entram no banco pelo deploy da main; depois disso rodar o seed demo (docs/AUTH.md)                                |
| 19/09/2026 | 4, 5           | Parte pura das Etapas 4 e 5 em `packages/engine` (`84b48f2`): normalização, tendência, persistência, metric/overall health, confiança, risco, prioridade, gatilhos, evidências, forecast e motor de SLA com 187 testes; docs SCORING, SLA_ENGINE e METRICS_ENGINE. Falta o acoplamento ao banco (módulos scoring e sla da API) |
| 19/09/2026 | 8              | Dashboard com dados de exemplo (`22020d9`): abas Em risco e Geral, forecast priorizado, ranking, filtros §61; tipos em `packages/shared/src/dashboard/`. Troca do mock pela API quando 4 e 6 fecharem. Integração da rodada em `9250706` (engine importa os tipos de forecast do shared)                                       |
| 19/09/2026 | 2              | Etapa 2 concluída: `portfolio_clients`, `plans` e `contracts` em Drizzle com RLS, módulos `portfolio-clients` e `contracts` na API (busca, filtros §61, arquivamento, contrato único ativo), tela `/clients`, aba Planos em `/settings` e `ContractsPanel` (`75d6826`). Guia em `docs/CLIENTS.md`                              |
| 19/09/2026 | 3              | Etapa 3 concluída: `metric_definitions`, `metric_models`, `metric_model_versions` e `metric_model_items` com RLS, validação Zod + motor, `/metrics` e `/metric-models` (versões, ativação com soma 100 %, rebalance, preview-score), telas `/metrics` e `/metrics/:id` com simulação (`ad9b1ee`). `docs/METRICS_ENGINE.md` §8  |
| 19/09/2026 | 7              | Núcleo puro do importador em `packages/importer` (`43b5562`): leitura XLSX/CSV/JSON, catálogo de datasets, sugestão de mapeamento, coerção, validação com relatório e preset da planilha do desafio. Falta o módulo `imports` da API e a tela `/import` (Rodada 3). Guia em `docs/IMPORTS.md`                                  |
| 19/09/2026 | 9              | Visão do cliente com dados de exemplo (`07cf297`): cabeçalho §57/§58, oito abas, evidências, recomendações, timeline e DTOs em `packages/shared/src/client-health/`. Troca do mock pela API quando o módulo `client-health` existir (Rodada 3)                                                                                 |
| 19/09/2026 | 11             | Etapa 11 concluída no fluxo manual (`9b35ed7`): upload para o bucket privado `documents`, extração de texto por tipo, sugestões com revisão humana, telas `/documents` e `/documents/:id`; `MetricExtractionProvider` pronto para o provider de IA (A5). Guia em `docs/DOCUMENTS.md`                                           |
| 19/09/2026 | 2, 3, 7, 9, 11 | Integração da Rodada 2 (`f95c989`): migration única `20260920005758_round_two_domain` (entra no banco pelo deploy da main), `ContractsPanel` no slot de contratos da visão do cliente e `/metrics` tratando a sugestão aceita em `/documents` (`state.prefill`)                                                                |
| 20/09/2026 | 11             | Documentos elevado a P0: planilhas importadas entram automaticamente no arquivo da organização; análise gera sugestões com evidência, confiança e limites via Claude quando há chave e por heurística local quando não há; revisão humana e criação da métrica permanecem obrigatórias.                                        |
| 20/09/2026 | 10             | `/metrics` virou a tela única do configurador: a lista de modelos saiu da barra e virou a faixa do alto (modelo, versão em vigor, soma dos pesos e **Publicar alterações**), e cada dado da tabela se edita no lugar — clique abre um bloquinho com as opções (tipo, direção, ativa, normalização) ou um campo em branco (nome, peso), e a ordem muda arrastando pela alça ou pelas setas. Cadastro salva na hora (`PATCH /metrics/:id`); peso, ordem e normalização viram rascunho e só valem ao publicar (rascunho → PATCH → activate). `/metric-models/:id` continua sendo a configuração avançada (faixas, gatilhos, redistribuição, histórico de versões).                          |
